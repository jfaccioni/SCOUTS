...
def analyse(widget, input_file, output_folder, cutoff_rule, by_marker, tukey,
    ...
    samples = []
    control = ''
    df = check_input(sample_list=sample_list, samples=samples, control=control,
    ...
    df_list = []
    for dataframe, m, s, n, c in yield_dataframes(log=f, df=df,
...
def check_input(sample_list, samples, control, input_file, widget):
