...
from PySide2.QtWidgets import (QApplication, QButtonGroup, QCheckBox, QDoubleSpinBox, QFileDialog, QFrame, QFormLayout,
                               QHBoxLayout, QVBoxLayout, QLabel, QLineEdit, QMainWindow, QMessageBox, QPushButton,
                               QRadioButton, QStackedWidget, QTableWidget, QTableWidgetItem, QWidget)
...
class SCOUTS(QMainWindow):
    ...
    def __init__(self):
        ...
        self.helpquit_frame.setGeometry(self.margin['left'],
                                        self.run_button.y() + 15, self.stretch(), 80)
        ...
        self.credits.setGeometry(self.margin['left'],
                                 self.widget_vposition(self.helpquit_frame) - 15, self.stretch(), 10)
        self.credits.setText('Juliano Luiz Faccioni (LabSinal/UFRGS 2018-2019)')
        self.credits.setStyleSheet(self.style['credits'])
        self.credits.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        ...
        self.sample_table.setGeometry(self.margin['left'],
                                      self.widget_vposition(self.samples_frame) + 5, self.stretch(), 380)
        self.sample_table.setColumnCount(2)
        n, m = QTableWidgetItem('Reference?'), QTableWidgetItem('Sample name')
        ...
        self.saveclear_frame.setGeometry(self.margin['left'],
                                         self.widget_vposition(self.sample_table) - 15, self.stretch(), 80)
    ...
    def write_to_sample_table(self):
        ...
        if sample:
            ...
            iscontrol = QTableWidgetItem(ref)
            iscontrol.setFlags(Qt.ItemIsEnabled)
            row_position = table.rowCount()
            table.insertRow(row_position)
            table.setItem(row_position, 1, sample)
            table.setItem(row_position, 0, iscontrol)
